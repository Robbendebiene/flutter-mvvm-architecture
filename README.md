...
1. **View Models should avoid exposing models directly.**
...
1. **View Models should not contain business logic.**
    Dos:
...
For user prompts call `request(Choice(...))` from the View Model and use the `ChoiceHandler` mixin from the extras directory.

For snack bars call `request(Choice(...))` from the View Model and use the `NotificationHandler` mixins from the extras directory.
